...
from nemo_gym.web.native_browser import (
    ...
    RESET_WAIT_UNTIL,
    NativeBrowserDriver,
    NativeBrowserResourcesServerConfig,
    _paste_unicode,
...
def _config(**updates: Any) -> NativeBrowserResourcesServerConfig:
    return NativeBrowserResourcesServerConfig.model_validate(
        {
            "name": "native-browser",
            "host": "localhost",
            "port": 8010,
...
def _driver(tmp_path: Path, **config_updates: Any) -> NativeBrowserDriver:
    return NativeBrowserDriver(_config(**config_updates), "session-1", WebArtifactStore(tmp_path))
...
def test_capture_handles_inactive_tab_and_title_failure(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    NativeBrowserDriver._configure_page(first)
...
def test_step_success_terminal_and_failures(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    assert result.info == {"native_status": "running"}
    ...
    assert result.info == {"native_status": "done"}
    ...
    assert result.info["native_status"] == "error"
...
def test_step_bounds_hook_failure_and_closed_target(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    class Driver(NativeBrowserDriver):
        closed = False
    ...
    driver = Driver(_config(action_delay_seconds=0), "session-1", WebArtifactStore(tmp_path))
    context = _Context()
    driver._context = context
...
def test_computer_action_validation_and_coordinate_helpers(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    NativeBrowserDriver._configure_pyautogui()
...
def test_text_typing_preserves_special_and_unicode_chunks(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    pasted: list[str] = []
    monkeypatch.setattr("nemo_gym.web.native_browser._paste_unicode", lambda _gui, text: pasted.append(text))
...
def test_clipboard_process_cleanup(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("nemo_gym.web.native_browser.os.killpg", lambda _pid, sig: signals.append(sig))
    _stop_clipboard_owner(running)
    assert signals == [signal.SIGTERM, signal.SIGKILL]
    assert running.wait_calls == 3

    monkeypatch.setattr(
        "nemo_gym.web.native_browser.os.killpg",
...
def test_paste_unicode_success_and_failures(monkeypatch: pytest.MonkeyPatch) -> None:
    pyautogui = _PyAutoGUI()
    monkeypatch.setattr("nemo_gym.web.native_browser.time.sleep", lambda _seconds: None)
    monkeypatch.setattr("nemo_gym.web.native_browser.shutil.which", lambda _name: "/usr/bin/xclip")
    ...
    monkeypatch.setattr("nemo_gym.web.native_browser.subprocess.Popen", lambda *_args, **_kwargs: process)
    monkeypatch.setattr("nemo_gym.web.native_browser._stop_clipboard_owner", lambda _process: None)
    ...
    monkeypatch.setattr("nemo_gym.web.native_browser.shutil.which", lambda _name: None)
    with pytest.raises(RuntimeError, match="xclip is required"):
        _paste_unicode(pyautogui, "雪")

    monkeypatch.setattr("nemo_gym.web.native_browser.shutil.which", lambda _name: "/usr/bin/xclip")
    monkeypatch.setattr(
        "nemo_gym.web.native_browser.subprocess.Popen",
    ...
    monkeypatch.setattr("nemo_gym.web.native_browser.subprocess.Popen", lambda *_args, **_kwargs: early)
