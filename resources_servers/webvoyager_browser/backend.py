...
"""WebVoyager-only policy for the shared headed native browser driver."""
...
from resources_servers.webvoyager_browser.captcha import (
...
class WebVoyagerBrowserDriver(NativeBrowserDriver):
    """Shared native browser specialized for WebVoyager's public sites."""
    ...
    def __init__(
        ...
        super().__init__(config, session_id, artifacts)
    ...
    def _proxy_for_task(self, task: WebTask) -> str:
        if task.benchmark != WebBenchmark.WEBVOYAGER:
            raise ValueError("webvoyager_browser only accepts WebVoyager tasks")
    ...
    def _configure_proxy_metadata(self, proxy_config: dict[str, str]) -> None:
        try:
            ...
            LOG.warning(
                "event=webvoyager_browser_proxy_metadata_failed session=%s task=%s",
                self.session_id,
    ...
    def _step_info(self, action: WebAction) -> dict[str, Any]:
        ...
        if self._browser_target_closed:
            return {"action_error": self._last_error, "native_status": BROWSER_TARGET_CLOSED_STATUS}
        if self._captcha_budget_exhausted:
            return {"action_error": self._last_error, "native_status": CAPTCHA_BUDGET_EXHAUSTED_STATUS}
...
def webvoyager_backend_factory(config, session_id: str, artifacts: WebArtifactStore):
    ...
    return ComposedWebBackend(
        WebVoyagerBrowserDriver(config, session_id, artifacts),
