...
"""WebArena-family URL resolution and login for the native visual driver."""
...
def configured_site_urls(task: WebTask) -> dict[str, str]:
    ...
    if missing:
        raise ValueError(f"missing native site URL environment variables: {', '.join(sorted(set(missing)))}")
    return urls
    ...
    for site in task.sites:
        ...
        for attempt in range(3):
            try:
                ...
                if attempt == 2:
                    raise
                LOG.warning("event=native_site_login_retry site=%s attempt=%d/3", site, attempt + 1)
    ...
    *,
    ...
    try:
        if site == "reddit":
            ...
            page.get_by_role("button", name="Log in").click()
        time.sleep(2)
        LOG.info("event=native_site_login_complete site=%s", site)
    finally:
        page.close()
