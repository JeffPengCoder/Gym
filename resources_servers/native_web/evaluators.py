...
"""Benchmark-specific evaluators for the native visual web runtime."""
...
class NativeBenchmarkEvaluator(Protocol):
    """One benchmark's scoring contract inside the native browser process."""
...
def _reference_task_config(
    ...
    if not isinstance(task_config.get("eval"), dict):
        raise EvaluatorConfigurationError("native WebArena-family task requires an eval object")
    if not isinstance(task_config["eval"].get("eval_types"), list):
        raise EvaluatorConfigurationError("native WebArena-family task requires eval.eval_types")
...
class NativeWebArenaFamilyEvaluator:
    ...
    def prepare(
        ...
        *,
        ...
        if _task_needs_judge(task_config) and not os.environ.get("WEBARENA_JUDGE_API_KEY", "").strip():
            raise EvaluatorConfigurationError("WEBARENA_JUDGE_API_KEY is required by this native WebArena-family task")
        if any(
        ...
        from resources_servers.native_web.reference_evaluation import (
        ...
        self._before = before
        LOG.info(
            "event=native_evaluator_prepared benchmark=%s task=%s collision_mitigation=%s snapshot_groups=%d",
            task.benchmark.value,
            task.task_id,
    ...
    def evaluate(
        ...
        *,
        ...
        if self._task_key != (task.benchmark, task.task_id):
            raise RuntimeError("native WebArena-family evaluator is not prepared for this task")
        if self._task_config is None or self._collision_plan is None or self._before is None:
            raise RuntimeError("native WebArena-family evaluator state is incomplete")
        ...
        judge_log_path = (
            browser_context.artifact_dir / "native-evaluator-judge.jsonl"
        ...
        if task.benchmark == WebBenchmark.WEBARENA:
            ...
            verifier_version = "native-webarena-3b775dc"
        else:
            ...
            verifier_version = "native-visualwebarena-3b775dc"
        score = max(0.0, min(1.0, float(score)))
        LOG.info(
            "event=native_evaluator_complete benchmark=%s task=%s score=%.3f message=%r",
            task.benchmark.value,
            task.task_id,
            score,
            message[:500],
...
class NativeTaskEvaluator:
    ...
    def __init__(
        self,
        evaluators: Mapping[WebBenchmark, NativeBenchmarkEvaluator] | None = None,
        *,
            ...
            if config is not None:
                for benchmark in (WebBenchmark.WEBARENA, WebBenchmark.VISUALWEBARENA):
                    if benchmark in config.allowed_benchmarks:
                        resolved_evaluators[benchmark] = NativeWebArenaFamilyEvaluator(benchmark, config)
            self._evaluators = resolved_evaluators
        else:
            self._evaluators = dict(evaluators)
        self._active: NativeBenchmarkEvaluator | None = None
        self._active_task: tuple[WebBenchmark, str] | None = None

    def _evaluator_for(self, task: WebTask) -> NativeBenchmarkEvaluator:
        evaluator = self._evaluators.get(task.benchmark)
        if evaluator is None:
            raise EvaluatorConfigurationError(
                f"native evaluator for benchmark {task.benchmark.value!r} is not installed"
            )
        return evaluator
    ...
    def prepare(
        ...
        *,
        ...
        if self._active is not None:
            raise RuntimeError("native evaluator is already bound to a task")
    ...
    def evaluate(
        ...
        *,
        ...
        if self._active is None or self._active_task != task_key:
            raise RuntimeError("native evaluator must be prepared for the active task before evaluation")
