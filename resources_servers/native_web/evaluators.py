...
def _reference_task_config(
    ...
    if not isinstance(task_config.get("eval"), dict):
        raise EvaluatorConfigurationError("native WebArena task requires an eval object")
    if not isinstance(task_config["eval"].get("eval_types"), list):
        raise EvaluatorConfigurationError("native WebArena task requires eval.eval_types")
    ...
    def prepare(
        ...
        *,
        ...
        if _task_needs_judge(task_config) and not os.environ.get("WEBARENA_JUDGE_API_KEY", "").strip():
            raise EvaluatorConfigurationError("WEBARENA_JUDGE_API_KEY is required by this native WebArena task")
    ...
    def evaluate(
        ...
        *,
        ...
        if self._task_key != (task.benchmark, task.task_id):
            raise RuntimeError("native WebArena evaluator is not prepared for this task")
        if self._task_config is None or self._collision_plan is None or self._before is None:
            raise RuntimeError("native WebArena evaluator state is incomplete")
...
class NativeTaskEvaluator:
    """Select the local WebArena evaluator without browser coupling.
        ...
        if evaluators is None:
            ...
            if config is not None:
                if WebBenchmark.WEBARENA in config.allowed_benchmarks:
                    resolved_evaluators[WebBenchmark.WEBARENA] = NativeWebArenaEvaluator(config)
